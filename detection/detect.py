...
class PNet(nn.Module):
    ...
    def __init__(self, pretrained):
        ...
        if pretrained:
            state_dict_path = os.path.join(pretrained, 'pnet.pt')
...
class RNet(nn.Module):
    ...
    def __init__(self, pretrained):
        ...
        if pretrained:
            state_dict_path = os.path.join(pretrained, 'rnet.pt')
            state_dict = torch.load(state_dict_path)
            self.load_state_dict(state_dict)
...
class ONet(nn.Module):
    ...
    def __init__(self, pretrained):
        ...
        if pretrained:
            state_dict_path = os.path.join(pretrained, 'onet.pt')
            state_dict = torch.load(state_dict_path)
            self.load_state_dict(state_dict)
...
class FaceDetector(nn.Module):
    ...
    def __init__(
        self, pretrained, image_size=160, margin=0, min_face_size=20,
